...
class Solution:
    def levelOrder(self, root: Optional[TreeNode]) -> List[List[int]]:
        ...
        # TC: O(n) -> Each node pushed and popped once, O(1) work each
        # SC: O(h) -> The stack holds up to O(h) nodes along a path (depth-first)
        # Same O(h) space as recursive DFS, avoiding recursion-depth limits. Like recursive DFS, it beats BFS's O(n) width space on balanced trees
