class MinStack:
    ...
    def __init__(self):
        # One stack for holding (value, min_so_far) tuples
        self.stack = []

    def push(self, val: int) -> None:
        # Compute the running min: smaller of val and the previous entry's stored min (or val if empty). We compare against self.stack[-1][1] — the previous entry's min, not its value.
        curMin = min(val, self.stack[-1][1] if self.stack else val)
        # Push the pair (val, curMin) as a single unit
        self.stack.append((val, curMin))

    def pop(self) -> None:
        # This removes the whole tuple at once
        self.stack.pop()

    def top(self) -> int:
        # return the first field [0] (the actual value)
        return self.stack[-1][0]

    def getMin(self) -> int:
        # return the second field [1] (the min at this level)
    ...
    # The index [0] vs [1] is how we pull apart the two fields. 
    # Bundling the min with each value means they can never desync on pop
