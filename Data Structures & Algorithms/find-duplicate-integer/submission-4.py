class Solution:
    def findDuplicate(self, nums: List[int]) -> int:
        ...
        while True:
            ...
            # Where they meet is the cycle entrance (the duplicate). The returned slow is the duplicate value. It just so happens that this value also equals the cycle entrance index
            if slow == slow2:
                return slow
        ...
        # What "slow" Holds Throughout -> Every time we do "slow = nums[slow]", slow becomes a value pulled from the array — which we then use as the next index. So slow is playing both roles as it moves. It's a value that immediately gets treated as the next index. The cycle entrance's index equals the duplicate value. So at the return point, slow holds a number that is simultaneously: the index where the cycle enters, AND the value that is duplicated
