class Solution:
    def characterReplacement(self, s: str, k: int) -> int:
        ...
        left = 0
        # This is to track the count of the most frequent character seen
        maxFreq = 0
        # Track the longest valid window
        longest = 0

        for right in range(len(s)):
            # Add the new character to the frequency count
            countMap[s[right]] = countMap.get(s[right], 0) + 1
            # Update the max frequency if this character is now the most common
            ...
            while (right - left + 1) - maxFreq > k:
                ...
                left += 1
            # Window is valid — record its size
