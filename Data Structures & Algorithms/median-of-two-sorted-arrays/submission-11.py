class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        # Ensure A is the smaller array — binary searching it is faster and keeps j in range. The swap isn't just an optimization — it's a correctness guard. It also tightens the complexity from O(log(max)) to O(log(min)).
        if len(B) < len(A):
            A, B = B, A
        ...
        # Why binary searching the smaller array -> If A were the LARGER array, i could be large enough that j = half - i - 2  goes very negative — far past -1. The sentinel only handles j == -1 gracefully (as "took nothing from B"). Deeply negative j would silently index B from the wrong end. Searching the smaller array bounds i tightly enough that j always lands in [-1, len(B)-1]. The swap isn't just an optimization — it's a correctness guard. It also tightens the complexity from O(log(max)) to O(log(min)).
