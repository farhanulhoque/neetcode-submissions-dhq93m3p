class Solution:
    def combinationSum(self, nums: List[int], target: int) -> List[List[int]]:
        ...
        # TC: O(n^(t/m)) -> n = number of candidates, t = target, and let the smallest number be m. combinations can be as deep as t/m (the smallest number repeated to reach the target), and each level branches up to n ways. The pruning (total > target) cuts many branches in practice, but the worst case stays exponential
        # SC: O(t/m) -> recursion depth = longest combination = at most t/m frames. The `combo` list holds at most t/m elements. 
        ...
        # The two base cases — success vs pruning -> Two base cases: total == target records a success, and total > target prunes a dead branch (since all numbers are positive, overshooting can never recover).The prune (total > target) is an OPTIMIZATION — it cuts off dead branches early instead of exploring combinations that can never work.

        # Why the prune relies on positive numbers -> The total > target prune is valid only because the numbers are positive — adding more can only grow the sum, so overshooting is irrecoverable. With negatives allowed, a later negative could bring the sum back to target, so this prune would wrongly discard valid paths. Always check the sign assumption before pruning on a sum.
