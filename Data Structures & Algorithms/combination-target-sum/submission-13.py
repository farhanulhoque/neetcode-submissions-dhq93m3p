class Solution:
    def combinationSum(self, nums: List[int], target: int) -> List[List[int]]:
        ...
        # TC: O(n · 2ⁿ) -> like Subsets, each element is fundamentally an include/exclude choice, giving up to 2ⁿ combinations, with an O(n) copy each. The sort (O(n log n)) is dominated by the exponential search. In practice, the skip rule and pruning cut the tree dramatically — but worst case stays exponential
