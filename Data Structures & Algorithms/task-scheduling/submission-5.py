class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        # TC: O(n) -> Counter: O(n) to count all tasks, The while loop runs once per CYCLE: O(n), Each cycle does heap ops of O(log k). Since k ≤ 26, log k ≤ ~5 = constant. total ≈ O(N) cycles × O(log k) per cycle. Because k ≤ 26 (constant), O(log k) = O(1), giving O(N) overall. Treating the 26-letter alphabet as a constant factor
        # SC: O(1) -> The heap holds at most k entries (distinct tasks) → ≤ 26 → O(1). The queue holds at most k entries too → O(1). Counter holds k entries → O(1). O(1) space (since k ≤ 26 is bounded)
