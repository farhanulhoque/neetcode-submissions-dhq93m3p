...
class Solution:
    def removeNthFromEnd(self, head: Optional[ListNode], n: int) -> Optional[ListNode]:
        ...
        # In removal, the dummy is a gateway to an existing list — it must point at head so the pointers can walk through the real nodes. ListNode() alone would give us a dummy pointing at None, disconnected from everything. In building, the dummy is a foundation to build on — it starts empty (next=None) and the list grows after it. There's no pre-existing list to point at.
        # if the list already exists and you need to traverse or modify it, connect the dummy to it (ListNode(0, head)). If you're building a fresh list, start the dummy empty (ListNode()). In Remove Nth Node, the nodes are already there — the dummy has to point at them, or left and right have nothing to walk through and return dummy.next gives back None.
