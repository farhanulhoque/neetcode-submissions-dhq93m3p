class Solution:
    def subsets(self, nums: List[int]) -> List[List[int]]:
        ...
        # SC: O(n) -> the recursion stack goes at most n deep (one frame per element) → O(n). The `subset` list holds at most n elements → O(n). O(n) extra space, NOT counting the output.
