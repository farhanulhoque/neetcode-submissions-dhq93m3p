class Solution:
    ...
    # Follow-up -> This length prefixing approach already works for any character set! The beauty of length prefixing is that we never scan the content of the string looking for a delimiter. We always know exactly how many characters to read. So whether the string contains unicode, emojis, # symbols, or any special character — we just skip over exactly length characters without caring what's inside.
    # In Python, len() counts characters, not bytes. But some characters take multiple bytes. So our encoding is safe in Python. But if you were sending this over a network and decoding in another language (e.g. C++ or Java), you'd need to be explicit about encoding. Encode to UTF-8 bytes, use byte length. Decode back from UTF-8 bytes.